...
if _GROQ_API_KEY:
    ...
    logger.info(f"[gemini] Generation model: {_GEN_MODEL} | Embed: {_EMBED_MODEL}")
...
def embed_text(text: str) -> list[float]:
    ...
    result = _client.models.embed_content(model=_EMBED_MODEL, contents=text[:8000])
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                if attempt < MAX_RETRIES:
                    import time
                    wait = 2 ** attempt
                    logger.warning(f"[gemini] Embed retry {attempt+1}/{MAX_RETRIES} chunk {i}: {e}")
                    time.sleep(wait)
        else:
